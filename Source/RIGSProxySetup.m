...
Class _RIGS_register_ruby_class (VALUE rb_class)
{
  ...
  if (ObjcUtilities_new_class(rb_class_name,"RIGSWrapObject", 0) == NO) {
      NSLog(@"Could not register class with ObjC: %s",rb_class_name);
      return nil; 
  }
  ...
  if (class == nil) {
      NSLog(@"Class was created but could not be loaded: %@",className);
      return nil; 
  }
                            ...
                            const char retValueType, int nbArgs, char *objcTypes)
{
  ...
  if ( rb_cvar_defined(rb_class, rb_ID) ) {
    ...
    if (TYPE(rb_objc_types) == T_HASH) {
      ...
      if (!NIL_P(value)) {
        strcpy(objcTypes, STR2CSTR(value));
